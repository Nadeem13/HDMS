...
import com.iustbatch2018rollnocse70.hdms.Chef;
...
public class ChefPreparedOrderView extends AppCompatActivity {
    ...
    Spinner Shipper;
    String deliveryId = "xpoNz3SE99hsjUIQXSP3qcBnS8F3";
    ...
    private void CheforderdishesView() {
        ...
        reference = FirebaseDatabase.getInstance().getReference("ShopkeeperFinalOrders").child(FirebaseAuth.getInstance().getCurrentUser().getUid()).child(RandomUID).child("Items");
                ...
                if (chefFinalOrdersList.size() == 0) {
                    ...
                    Prepared.setOnClickListener(new View.OnClickListener() {
                        ...
                        public void onClick(View v) {
                            ...
                            DatabaseReference data = FirebaseDatabase.getInstance().getReference("Shopkeeper").child(FirebaseAuth.getInstance().getCurrentUser().getUid());
                            data.addListenerForSingleValueEvent(new ValueEventListener() {
                                @Override
                                public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                                    final Chef chef = dataSnapshot.getValue(Chef.class);
                                    final String ChefName = chef.getFname() + " " + chef.getLname();
                                    DatabaseReference databaseReference1 = FirebaseDatabase.getInstance().getReference("ShopkeeperFinalOrders").child(FirebaseAuth.getInstance().getCurrentUser().getUid()).child(RandomUID).child("Items");
                                    databaseReference1.addListenerForSingleValueEvent(new ValueEventListener() {
                                        ...
                                        public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                                            for (DataSnapshot dataSnapshot1 : dataSnapshot.getChildren()) {
                                                ...
                                                String dishid = chefFinalOrders.getItemId();
                                                userid = chefFinalOrders.getUserId();
                                                hashMap.put("ShopkeeperId", chefFinalOrders.getShopkeeperId());
                                                hashMap.put("ItemId", chefFinalOrders.getItemId());
                                                hashMap.put("ItemName", chefFinalOrders.getItemName());
                                                hashMap.put("ItemPrice", chefFinalOrders.getItemPrice());
                                                hashMap.put("ItemQuantity", chefFinalOrders.getItemQuantity());
                                                hashMap.put("RandomUID", RandomUID);
                                                hashMap.put("TotalPrice", chefFinalOrders.getTotalPrice());
                                                hashMap.put("UserId", chefFinalOrders.getUserId());
                                                FirebaseDatabase.getInstance().getReference("DeliveryShipOrders").child(deliveryId).child(RandomUID).child("Items").child(dishid).setValue(hashMap);
                                            }
                                            DatabaseReference data = FirebaseDatabase.getInstance().getReference("ShopkeeperFinalOrders").child(FirebaseAuth.getInstance().getCurrentUser().getUid()).child(RandomUID).child("OtherInformation");
                                            data.addListenerForSingleValueEvent(new ValueEventListener() {
                                                @Override
                                                public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
        ...
        DatabaseReference databaseReference = FirebaseDatabase.getInstance().getReference("ShopkeeperFinalOrders").child(FirebaseAuth.getInstance().getCurrentUser().getUid()).child(RandomUID).child("OtherInformation");
